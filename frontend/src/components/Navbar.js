...
import { logout, selectIsAuthenticated } from '../features/authentication/authSlice';
  ...
  return (
    <div className='shadow py-5 sticky top-0 bg-white'>
        <nav className='flex justify-between mx-20 h-8 items-center'>
            <Link to='/' onClick={() => handleCategoryClick('all')}><h1 className='font-extrabold text-3xl'>TIME BLOGGER</h1></Link>
            <div className=' uppercase text-metal font-semibold'>
              <ul className='flex gap-10'>
                ...
                <li className='cursor-pointer' onClick={() => handleCategoryClick('Personal Development')}>Personal Development</li>
                <li className='cursor-pointer' onClick={() => handleCategoryClick('Travel')}>Travel</li>
